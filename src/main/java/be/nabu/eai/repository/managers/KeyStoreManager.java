...
public class KeyStoreManager implements ArtifactManager<DefinedKeyStore> {
	...
	@Override
	public DefinedKeyStore load(ResourceEntry entry, List<Validation<?>> messages) throws IOException, ParseException {
		return new DefinedKeyStore(entry.getId(), entry.getContainer());
	}

	@Override
	public List<Validation<?>> save(ResourceEntry entry, DefinedKeyStore artifact) throws IOException {
		try {
	...
	@Override
	public List<Validation<?>> updateReference(DefinedKeyStore artifact, String from, String to) throws IOException {
		return null;
	}
}
